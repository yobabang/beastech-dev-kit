...
//NM唤醒报文
TCAN NMWakeUpMsg = { Diag_ch,1,8,0,NM_WakeUp_ID,0,{0x3F,0x40,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF} };
TCAN NMWakeUpMsg_NMChannel = { Diag_ch,1,8,0,NM_WakeUp_ID,0,{0x3F,0x40,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF} };

s32 NMWakeUp(u8 wakeup)
{
	if (wakeup == 1)
	{
		NMWakeUpMsg.FIdxChn = NetworkCluster[TpBusName_Temp];
		NMWakeUpMsg_NMChannel.FIdxChn = NetworkCluster[TpNMBusName_Temp];
		com.add_cyclic_message_can(&NMWakeUpMsg, 100); 
		com.add_cyclic_message_can(&NMWakeUpMsg_NMChannel, 100);
		return 0;
	}
	else
	{
		com.del_cyclic_message_can(&NMWakeUpMsg);
		com.del_cyclic_message_can(&NMWakeUpMsg_NMChannel);
		return -1;
	}
}
//如果等到了报文，ret=0
...
s32 CAN_TP_TesterPresent(u8 Testpresent, u64 PeriodTime = Diag_UDS_S3Client)
{
	if (Testpresent == 1)
	{
		...
		{
			TesterPresentFD.FIdentifier = Diag_Function_ID;
			TesterPresentFD.FIdxChn = NetworkCluster[TpBusName_Temp];
			TesterPresentFD.is_brs = true;
