...
struct Available2FDID
{
    ...
    // 新增字段
    u8 SupportedIOParams; // 位掩码：Bit0=0x00(Return), Bit1=0x01(Reset), Bit2=0x02(Freeze), Bit3=0x03(ShortTerm)
    u8 STA_DataSize;      // 仅用于 0x03 (Short Term Adjustment) 的后续数据长度
};
// 初始化时建议将 SupportedIOParams 置为 0
Available2FDID IODIDList[200] = { 0 };
...
struct AvailableRID
{
    ...
    u8 Sup_Def; // 之前漏了 Def
    u8 Sup_PBL;
    u16 RequestLength; // 新增：输入参数长度 (用于 31 01)
};
...
AvailableService SupportServices[40] = {
    ...
    { 0x22,{},1},
    { 0x27,{{0x01,1,0,1,1} ,{0x03,1,1,1,1} ,{0x05,1,1,1,1},{0x11,1,1,1,1},{0x19,1,1,1,1}} , 5},
    {0x2E,{},1},
    {0x23,{},1},
    {0x28,{},1},
    {0x2F,{},1},
...
AvailableService SupportServices_Security[6] = {
    { 0x10, {{0x01,1,1,1,1},{0x02,1,1,1,1},{0x03,1,1,0,0}} ,1 },
    { 0x11, {{0x01,1,1,1,1},{0x03,1,1,1,1}},1 },
    { 0x14,{},1},
    { 0x19,{{0x02,1,1,0,0},{0x03,1,1,0,0},{0x04,1,1,0,0},{0x06,1,1,0,0},{0x0A}},1},
...
AvailableReadDID ReadDIDList[100] = {

};
//AvailableReadDID ReadDIDList[100] = {
//	{ 0x049A, 1, 0, 1, 1, 1, 0x01, 0x03 },
//	{ 0xF1AB, 11,0, 1, 1, 1, 0x01, 0x03 },  //1022499 & 1022500
//    { 0xF18A, 9, 1, 1, 0, 1, 0x01, 0x03 },
//    { 0x0350, 1, 0, 1, 0, 1, 0x01, 0x03 }   //3号元素是跑不跑1350898
//};

u8 readdidsize = 100;

AvailableWriteDID WriteDIDList[100] = {

};
//AvailableWriteDID WriteDIDList[50] = {
//    { 0xF186, 1, 1, 1, 1, 0x01, 0x03 },
//    { 0xF186, 8, 1, 1, 1, 0x01, 0x03 },
//};

u8 writedidsize = 100;

Available2CDID DefineDIDList[100] = {
...
AvailableRID RIDList_Phy_Default[50];
//AvailableRID RIDList_Phy_Default[50] = {
//    //{ 0x0206,1,1,0,0,0,0},
//    { 0x0206,1,1,0,0,0x01,256},
//    { 0x0413,1,1,0,0,0x01,256},
//    { 0x4000,1,1,0,0,0x01,256},
//    { 0x404C,1,1,0,0,0x01,256},
//    { 0x4058,1,1,0,0,0x01,256},
//    { 0x405A,1,1,0,0,0x01,256},
//    { 0x405E,1,1,0,0,0x01,256},
//    { 0x4289,1,1,0,0,0x01,256},
//};

AvailableRID RIDList_Phy_Extend[50];
//AvailableRID RIDList_Phy_Extend[50] = {
//
//    { 0x0206,1,1,0,0,0x01,256}, //31---0x01
//    //27(0x33)
//    { 0x0207,1,1,0,0,0x01,256},
//    //switch 10---0x03 and 27(0x05)
//    { 0x0208,1,1,0,0,0x01,256},
//    { 0x0407,2,1,0,0,0x01,256},  //31---0x02 0x03 0x01
//    { 0x0408,1,1,0,0,0x01,256},  //
//    { 0x0413,1,1,0,0,0x01,256},
//    { 0x4000,1,1,0,0,0x01,256},
//    //27(0x31) 
//    { 0x4003,1,1,0,0,0x01,256},
//    { 0x4010,1,1,0,0,0x01,256},
//};
AvailableRID RIDList[100] = {
...
//};
u8 ridsize = 100;
u8 ridsize_default = 8;
u8 ridsize_extend = 9;

struct AvailableDTC DTCList[5000] = {
...
void GetSDDBJSONFilePath(char** path)
{
    ...
    strcat(*path, "01_Input\\SDDB\\sddb.json");

    log("sddb.json path is %s", *path);
}
...
int sddb_json_reader(char* jsonpath) {
    ...
    for (int i = 0; i < SW_size; i++)
    {
        ...
        //    BusTypeNow = 2;
        //}
        BusTypeNow = (strcmp(Type->valuestring, "CAN") == 0) ? 1 : 2;
        ...
        for (int j = 0; j < Service_size; j++)
        {
            ...
            char* str;
            if (strncmp(SID->valuestring, "27", 2) == 0)
                continue;
            ...
            if (Subfunctions == NULL || Subfunctions->type != cJSON_Object) // 确保子功能不为空并且是一个对象
            {
                ...
                if (strcmp(SID->valuestring, "22") == 0)
                {
                    ...
                    if (DataIdentifier_Array != NULL && DataIdentifier_Array->type == cJSON_Array)
                    {
                        ...
                        for (int iii = 0; iii < DataIdentifier_size; iii++)
                        {
                            ...
                            cJSON* SizeObj = cJSON_GetObjectItem(DataIdentifier, "@Size");
                            if (SizeObj) {
                                ReadDIDList[iii].DIDLength = atoi(SizeObj->valuestring);
                            }
                            else {
                                ReadDIDList[iii].DIDLength = 1; // 默认值
                            }
                ...
                if (strcmp(SID->valuestring, "2E") == 0)
                {
                    cJSON* DataIdentifiers = cJSON_GetObjectItem(Sub_Services, "DataIdentifiers");
                ...
                if (strcmp(SID->valuestring, "2F") == 0)
                {
                    cJSON* DataIdentifiers = cJSON_GetObjectItem(Sub_Services, "DataIdentifiers");
                    // 有时候 DataIdentifiers 下面直接就是 Array，有时候还有一层 DataIdentifier
                    cJSON* DataIdentifier_Array = cJSON_GetObjectItem(DataIdentifiers, "DataIdentifier");

                    if (DataIdentifier_Array != NULL)
                    {
                        // 如果只有一个DID，cJSON可能解析为Object而不是Array
                        int DataIdentifier_size = 0;
                        if (DataIdentifier_Array->type == cJSON_Array) {
                            DataIdentifier_size = cJSON_GetArraySize(DataIdentifier_Array);
                        }
                        else if (DataIdentifier_Array->type == cJSON_Object) {
                            DataIdentifier_size = 1;
                        }
                        ...
                        for (int iii = 0; iii < DataIdentifier_size; iii++)
                        {
                            // 1. 获取当前的 DataIdentifier 对象
                            cJSON* DataIdentifier = NULL;
                            if (DataIdentifier_Array->type == cJSON_Array) {
                                DataIdentifier = cJSON_GetArrayItem(DataIdentifier_Array, iii);
                            }
                            else {
                                DataIdentifier = DataIdentifier_Array;
                            }

                            if (DataIdentifier == NULL) continue;

                            cJSON* DID_Item = cJSON_GetObjectItem(DataIdentifier, "@ID");
                            if (DID_Item != NULL && DID_Item->valuestring != NULL)
                            {
                                char* endptr;
                                // 使用 strtoul 解析无符号长整型，基数16
                                IODIDList[iii].DID = strtoul(DID_Item->valuestring, &endptr, 16);
                            }
                            else
                            {
                                log("Error: DID ID not found at index %d", iii);
                                continue;
                            }

                            // 初始化字段
                            IODIDList[iii].SupportedIOParams = 0;
                            IODIDList[iii].STA_DataSize = 1;

                            cJSON* DataParameters = cJSON_GetObjectItem(DataIdentifier, "DataParameters");
                            if (DataParameters != NULL)
                            {
                                cJSON* DataParam_Array = cJSON_GetObjectItem(DataParameters, "DataParameter");

                                if (DataParam_Array != NULL)
                                {
                                    int param_count = 0;
                                    if (DataParam_Array->type == cJSON_Array) {
                                        param_count = cJSON_GetArraySize(DataParam_Array);
                                    }
                                    else if (DataParam_Array->type == cJSON_Object) {
                                        param_count = 1;
                                    }

                                    for (int p = 0; p < param_count; p++)
                                    {
                                        cJSON* ParamItem = NULL;
                                        if (DataParam_Array->type == cJSON_Array)
                                            ParamItem = cJSON_GetArrayItem(DataParam_Array, p);
                                        else
                                            ParamItem = DataParam_Array;

                                        cJSON* ParamID = cJSON_GetObjectItem(ParamItem, "@ID");
                                        if (ParamID == NULL || ParamID->valuestring == NULL) continue;

                                        // 解析支持的参数类型
                                        if (strcmp(ParamID->valuestring, "00") == 0) {
                                            IODIDList[iii].SupportedIOParams |= 0x01;
                                        }
                                        else if (strcmp(ParamID->valuestring, "01") == 0) {
                                            IODIDList[iii].SupportedIOParams |= 0x02;
                                        }
                                        else if (strcmp(ParamID->valuestring, "02") == 0) {
                                            IODIDList[iii].SupportedIOParams |= 0x04;
                                        }
                                        else if (strcmp(ParamID->valuestring, "03") == 0) {
                                            IODIDList[iii].SupportedIOParams |= 0x08;

                                            // 解析 03 后面的数据长度
                                            cJSON* STA_DataParams = cJSON_GetObjectItem(ParamItem, "DataParameters");
                                            if (STA_DataParams) {
                                                cJSON* STA_InnerParam = cJSON_GetObjectItem(STA_DataParams, "DataParameter");
                                                if (STA_InnerParam) {
                                                    // 如果是数组，取第一个；如果是对象，直接取
                                                    cJSON* TargetParam = (STA_InnerParam->type == cJSON_Array) ? cJSON_GetArrayItem(STA_InnerParam, 0) : STA_InnerParam;

                                                    cJSON* SizeObj = cJSON_GetObjectItem(TargetParam, "@Size");
                                                    if (SizeObj && SizeObj->valuestring) {
                                                        IODIDList[iii].STA_DataSize = atoi(SizeObj->valuestring);
                                                    }
                                                }
                                            }
                                        }
                            ...
                            log("DID: 0x%X, Supported Mask: 0x%02X, 03 Data Size: %d",
                                IODIDList[iii].DID, IODIDList[iii].SupportedIOParams, IODIDList[iii].STA_DataSize);
            ...
            if (strcmp(SID->valuestring, "19") == 0)
            {
                ...
                for (int p = 0; p < DTC_size; p++)
                {
                    ...
                    DTCList[p].DTC_Byte[0] = (DTCList[p].DTC >> 16) & 0xFF;
                    DTCList[p].DTC_Byte[1] = (DTCList[p].DTC & 0x00FF00) >> 8;
                    DTCList[p].DTC_Byte[2] = (DTCList[p].DTC & 0x0000FF);
    ...
    log("mcSupportServices[5].service:0x%x", SupportServices[5].service);//这里获取到了

    return 0;
}


//by mc 打印SupportServices结构体内容及大小
void printSupportServices() {
    int i, j;
    int total_services = 16; // 实际定义的服务数量

    printf("=== SupportServices Information ===\n");
    printf("Total services defined: %d\n\n", total_services);

    for (i = 0; i < total_services; i++) {
        printf("Service 0x%02X:\n", SupportServices[i].service);
        printf("  Subfunction count: %d\n", SupportServices[i].subfuncsize);

        if (SupportServices[i].subfuncsize > 0) {
            printf("  Subfunctions:\n");
            for (j = 0; j < SupportServices[i].subfuncsize; j++) {
                printf("    Subfunc 0x%02X: Def=%d, Ext=%d, PBL=%d, SBL=%d\n",
                    SupportServices[i].subfunc[j].Subfunc,
                    SupportServices[i].subfunc[j].Sup_Def,
                    SupportServices[i].subfunc[j].Sup_Ext,
                    SupportServices[i].subfunc[j].Sup_PBL,
                    SupportServices[i].subfunc[j].Sup_SBL);
            }
        }
        else {
            printf("  No subfunctions\n");
        }
        printf("\n");
    }
    ...
    printf("Size of AvailableService struct: %zu bytes\n", sizeof(struct AvailableService));
    printf("Size of AvaliableSubfunctions struct: %zu bytes\n", sizeof(struct AvaliableSubfunctions));
    printf("Total array size: %zu bytes\n", sizeof(SupportServices));
}

// 辅助函数：在指定目录下查找第一个文件名包含某关键字的 .sddb 文件
// 参数：dirPath - 目录路径（如 "C:\\...\\01_Input\\SDDB\\"）
//       keyword - 要匹配的关键字（"ZCUD" 或 "ZCUP"）
//       outFullPath - 输出缓冲区，存放找到的完整文件路径
// 返回值：1 找到，0 未找到
int FindSDDBFileByKeyword(const char* dirPath, const char* keyword, char* outFullPath, size_t outSize) {
    char searchPath[512];
    snprintf(searchPath, sizeof(searchPath), "%s*.sddb", dirPath);

    WIN32_FIND_DATAA findData;
    HANDLE hFind = FindFirstFileA(searchPath, &findData);
    if (hFind == INVALID_HANDLE_VALUE) {
        return 0; // 没有 .sddb 文件或目录不存在
    }

    int found = 0;
    do {
        if (strstr(findData.cFileName, keyword) != NULL) {
            snprintf(outFullPath, outSize, "%s%s", dirPath, findData.cFileName);
            found = 1;
            break;
        }
...
void SDDB_Parser()
{
    //char INI_FILE_PATH[200] = "input\\write_sddb.ini";
    //char SDDB_FILE_PATH[200] = "input\\E245_2422_24M08_ZCUDM_DVA.sddb";
    //char SDDB_JSON_PATH[200] = "input\\sddb.json";
    ...
    char dirPath[500];
    snprintf(dirPath, sizeof(dirPath), "%s01_Input\\SDDB\\", configPath);

    const char* keyword = (gCEMFlg == 1) ? "ZCUD" : "ZCUP";
    if (FindSDDBFileByKeyword(dirPath, keyword, SDDB_FILE_PATH, sizeof(SDDB_FILE_PATH))) {
        printf("SDDB PATH IS %s (matched by keyword '%s')\n", SDDB_FILE_PATH, keyword);
    }
    else {
        // 未找到匹配文件，回退到原硬编码逻辑
        printf("Warning: No SDDB file containing '%s' found in %s\n", keyword, dirPath);
        if (gCEMFlg == 1) {
            snprintf(SDDB_FILE_PATH, sizeof(SDDB_FILE_PATH), "%s01_Input\\SDDB\\ZCUD.sddb", configPath);
        }
        else {
            snprintf(SDDB_FILE_PATH, sizeof(SDDB_FILE_PATH), "%s01_Input\\SDDB\\ZCUP.sddb", configPath);
        }
        printf("Fallback to hardcoded path: %s\n", SDDB_FILE_PATH);
    }
    printf("SDDB PATH IS %s", SDDB_FILE_PATH);

    GetSDDBJSONFilePath(&jsonptr);

    //printf("sddb file path = %s,json path =  %s , 123", SDDB_FILE_PATH, jsonptr);
    ...
    for (int i = 0; i < servicesize; i++) {
        printf("Service 0x%02X:", SupportServices[i].service);
        printf("  Subfunction count: %d", SupportServices[i].subfuncsize);
        if (SupportServices[i].service == 0x27)
        {
            continue;
            if (SupportServices[i].subfuncsize > 0) {
                for (int j = 0; j < SupportServices[i].subfuncsize; j++) {

                    if (SupportServices[i].subfunc[j].Subfunc == 0x01 || SupportServices[i].subfunc[j].Subfunc == 0x02) {
                        SupportServices[i].subfunc[j].Sup_Def = 1;
                        SupportServices[i].subfunc[j].Sup_Ext = 0;
                        SupportServices[i].subfunc[j].Sup_PBL = 0;
                        SupportServices[i].subfunc[j].Sup_SBL = 0;
                    }
                    if (SupportServices[i].subfunc[j].Subfunc == 0x03 || SupportServices[i].subfunc[j].Subfunc == 0x04) {
                        SupportServices[i].subfunc[j].Sup_Def = 0;
                        SupportServices[i].subfunc[j].Sup_Ext = 1;
                        SupportServices[i].subfunc[j].Sup_PBL = 0;
                        SupportServices[i].subfunc[j].Sup_SBL = 0;
                    }
                    if (SupportServices[i].subfunc[j].Subfunc == 0x05 || SupportServices[i].subfunc[j].Subfunc == 0x06) {
                        SupportServices[i].subfunc[j].Sup_Def = 0;
                        SupportServices[i].subfunc[j].Sup_Ext = 1;
                        SupportServices[i].subfunc[j].Sup_PBL = 0;
                        SupportServices[i].subfunc[j].Sup_SBL = 0;
                    }
                    if (SupportServices[i].subfunc[j].Subfunc == 0x11 || SupportServices[i].subfunc[j].Subfunc == 0x12) {
                        SupportServices[i].subfunc[j].Sup_Def = 0;
                        SupportServices[i].subfunc[j].Sup_Ext = 1;
                        SupportServices[i].subfunc[j].Sup_PBL = 0;
                        SupportServices[i].subfunc[j].Sup_SBL = 0;
                    }
                    if (SupportServices[i].subfunc[j].Subfunc == 0x19 || SupportServices[i].subfunc[j].Subfunc == 0x1A) {
                        SupportServices[i].subfunc[j].Sup_Def = 0;
                        SupportServices[i].subfunc[j].Sup_Ext = 1;
                        SupportServices[i].subfunc[j].Sup_PBL = 0;
                        SupportServices[i].subfunc[j].Sup_SBL = 0;
