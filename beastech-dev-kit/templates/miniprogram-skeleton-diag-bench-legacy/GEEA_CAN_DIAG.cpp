...
// CODE BLOCK BEGIN Timer TestStepValueByIndex MTAw
// 定时器文档 "TestStepValueByIndex"
TMPTimerMS TestStepValueByIndex;
// CODE BLOCK END Timer TestStepValueByIndex


// CODE BLOCK BEGIN Global_Definitions 
#include <TestCase_CAN_DTC.h>
TCAN TempMessage;
...
void on_can_rx_NewOn_CAN_Rx1(const TCAN* ACAN) { try {  // 针对标识符 = 任意
	...
	//	return;
	//else {
	//	app.set_system_var_int32("CheckCom", 1);
	//}
...
void on_canfd_rx_NewOn_CAN_FD_Rx1(const TCANFD* ACANFD) { try {  // 针对标识符 = 任意 (FD)
	...
	if (ACANFD->FIdxChn == CH1)
		return;
	if ((ACANFD->FIdxChn != DUTBusoffChannel) && (DUTBusoffChannel != 99))
		return;
	...
	if (v == 1)
		...
		log("MessageID is %d, Chn is CH%d", TempMessageFD.FIdentifier, TempMessageFD.FIdxChn+1);
		log("set Checkcom 1");
		app.set_system_var_int32("CheckCom", 1);
	}
...
void on_canfd_pretx_NewOn_CAN_FD_PreTx1(const PCANFD ACANFD) { try {  // 针对标识符 = 任意 (FD)
 ...
 //   {
	...
	////CarConfig
	//if (CarConfigFlg) {
	//	if (carConfigData[0].FFrameIndentifier == carConfigData[1].FFrameIndentifier && carConfigData[0].FFrameIndentifier == carConfigData[2].FFrameIndentifier) {
	//		ACANFD->FData[carConfigData[0].carConfigSignalGroupData[0].StartBit / 8] = 
	//	}
	//}
...
void on_ethernet_rx_NewOn_Ethernet_Rx1(const TEthernetHeader* AEthernetHeader) { try {  // 针对报文 = 任意
...
void on_ethernet_tx_NewOn_Ethernet_Tx1(const TEthernetHeader* AEthernetHeader) { try {  // 针对报文 = 任意
...
void on_ethernet_pretx_NewOn_Ethernet_PreTx1(const PEthernetHeader AEthernetHeader) { try {  // 针对报文 = 任意
...
void on_tcp_listen_NewOn_TCP_Listen1(const native_int ASocket, native_int AClientSocket, const native_int AResult) { try {  // CH1: 192.168.1.111:1234
...
// CH1: 192.168.1.111:1234
void on_tcp_connect_NewOn_TCP_Connect1(const native_int ASocket, const native_int AResult) { try {  // CH1: 192.168.1.111:1234
...
// CH1: 192.168.1.111:1234
void on_tcp_close_NewOn_TCP_Close1(const native_int ASocket, const native_int AResult) { try {  // CH1: 192.168.1.111:1234
...
// CH1: 192.168.1.111:1234
void on_tcp_receive_NewOn_TCP_Receive1(const native_int ASocket, const native_int AResult, const u32 AAddr, const u32 APort, const pu8 AData, const native_int ASize) { try {  // CH1: 192.168.1.111:1234
...
// CH1: 192.168.1.111:1234
void on_tcp_transmit_NewOn_TCP_Transmit1(const native_int ASocket, const native_int AResult, const pu8 AData, const native_int ASize) { try {  // CH1: 192.168.1.111:1234
...
// CH1: 192.168.1.111:1234
void on_udp_receive_from_NewOn_UDP_Receive_From1(const native_int ASocket, const native_int AResult, const u32 AAddr, const u32 APort, const pu8 AData, const native_int ASize) { try {  // CH1: 192.168.1.111:1234
...
// CH1: 192.168.1.111:1234
void on_udp_transmit_to_NewOn_UDP_Transmit_To1(const native_int ASocket, const native_int AResult, const pu8 AData, const native_int ASize) { try {  // CH1: 192.168.1.111:1234
...
void on_var_change_On_TestSystem_CsvStatus(void) { try { // 变量 = TestSystem.CsvStatus


} catch (...) { log_nok("CRASH detected"); app.terminate_application(); }}
// CODE BLOCK END On_Var_Change On_TestSystem_CsvStatus

// CODE BLOCK BEGIN On_Timer T_StepUpTest TmV3VGltZXIx
// 定时器触发事件 "T_StepUpTest" for Timer NewTimer1
void on_timer_T_StepUpTest(void) { try { // 定时器 = NewTimer1
		printf("start Timer");
		char Stepdespriction[256];
		gtestIndex++;

		// 根据 gReadEndFlag 判断主线程是否处理完上一次数据
		if (gReadEndFlag)
		{
			// 根据当前模式打印不同的日志
			if (gCurrentTestMode == MODE_STEP_UP) {
				snprintf(Stepdespriction, sizeof(Stepdespriction), "--------------%dth cycle test of FDC10 Step Up------------", gtestIndex);
			}
			else if (gCurrentTestMode == MODE_STEP_DOWN) {
				snprintf(Stepdespriction, sizeof(Stepdespriction), "------------------%dth cycle test of FDC10 Step Down---------------------", gtestIndex);
			}
			else {
				// 异常情况，未设置模式
				snprintf(Stepdespriction, sizeof(Stepdespriction), "Unknown Test Mode in Timer!");
			}
...
// CODE BLOCK END On_Timer T_StepUpTest
...
// CODE BLOCK BEGIN On_Timer TestStepValue VGVzdFN0ZXBWYWx1ZUJ5SW5kZXg_
// 定时器触发事件 "TestStepValue" for Timer TestStepValueByIndex
void on_timer_TestStepValue(void) { 
	try { // 定时器 = TestStepValueByIndex

		if (1 == gReadEndFlag) {
			++gtestIndex;
			printf("gtestIndex is %d", gtestIndex);
		}
	} 
	catch (...) { log_nok("CRASH detected"); app.terminate_application(); }
}
// CODE BLOCK END On_Timer TestStepValue
