...
typedef s32(__stdcall* Tmetric_reset_can_frame_interval_stat)(const TLIBApplicationChannelType ABusType, const s32 AIdxChn, const u64 AFrameId);
...
typedef struct _TTSApp {
    ...
    native_int FDummy[534]; // >>> mp app end <<<
...
typedef struct _TTSCOM {
    ...
    native_int FDummy[521]; // >>> mp com end <<<
...
typedef struct _TTSTest {
    ...
    native_int FDummy[142]; // >>> mp test end <<<
...
typedef struct _TTSMBD {
    ...
    native_int FDummy[732]; // >>> mp mbd end <<<
} TTSMBD, * PTSMBD;
