...
│   └── capl-to-c-mapping.md      # CAPL→TSMaster C 转换参考（CAN/LIN/FlexRay/SysVar 函数映射 + 样板工程）
├── templates/
│   ├── miniprogram-skeleton/     # 可复制的完整可编译 SDK 工程（1.9M）
│   ├── tsmaster-sdk-header.h     # 官方 TSMaster SDK C 头文件（4347 行，从 TSMaster316 安装目录提取）
│   └── capl-to-c/                # CAPL→C 转换样板（SysAndCan兼容层 / MiniProgram骨架 / 函数映射Excel）
├── assets/
...
### 查协议事实
UDS 服务/NRC/会话/寻址 → `references/uds-services.md`。SWDL 流程 → `swdl-flow.md`。NM 状态机/PNC → `nm-state-machine.md`。**NM 工程实战范式**（config.ini/多通道/动作库）→ `nm-test-framework.md`。CC → `cc-comm-control.md`。E2E → `e2e-profile.md`。**LIN**（协议/LDF/动作库）→ `lin-communication.md`。**.NET Interop API**（外部脚本/CI 集成）→ `tsmaster-dotnet-api.md`。**CAPL→C 转换**（CANoe 迁移）→ `capl-to-c-mapping.md` + `templates/capl-to-c/`。**原始 C API 头文件** → `templates/tsmaster-sdk-header.h`。
...
| 新 CAPL 函数映射 | `references/capl-to-c-mapping.md` + 追加 Excel |
| 新 CAPL→C 样板代码 | `templates/capl-to-c/SysAndCan.h/.cpp` |
