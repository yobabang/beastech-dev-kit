...
### 查协议事实
UDS 服务/NRC/会话/寻址 → `references/uds-services.md`。SWDL 流程 → `swdl-flow.md`。NM 状态机/PNC → `nm-state-machine.md`。**NM 工程实战范式**（config.ini/多通道/动作库）→ `nm-test-framework.md`。CC → `cc-comm-control.md`。E2E → `e2e-profile.md`。**LIN**（协议/LDF/动作库）→ `lin-communication.md`。**.NET Interop API**（外部脚本/CI 集成）→ `tsmaster-dotnet-api.md`。**原始 C API 头文件** → `templates/tsmaster-sdk-header.h`。
