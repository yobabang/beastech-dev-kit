...
│   └── capl-resources/           # CAPL 资源：函数快速索引 Excel（1669 函数）+ SysAndCan兼容层
├── assets/
│   └── sample-test-case.h        # 范本用例（test_case_1021808 注释版）
└── sdks/
    └── x86/                      # TSMaster316 SDK 二进制（脱离安装目录的随身副本）
        ├── TSMaster.dll          # 1.1M — TSMaster 主程序 DLL（native）
        ├── Interop.TSMasterAPI.dll # 127K — .NET C# 互操作桥
        └── GenKeyX86Wrapper.exe  # 70K — 安全访问密钥生成器
```
...
- 从 CANoe CAPL 迁移测试用例到 TSMaster 时
- 写 CAPL 脚本，查 CANoe CAPL 函数/事件/语法时
...
### 查协议事实
UDS 服务/NRC/会话/寻址 → `references/uds-services.md`。SWDL 流程 → `swdl-flow.md`。NM 状态机/PNC → `nm-state-machine.md`。**NM 工程实战范式**（config.ini/多通道/动作库）→ `nm-test-framework.md`。CC → `cc-comm-control.md`。E2E → `e2e-profile.md`。**LIN**（协议/LDF/动作库）→ `lin-communication.md`。**.NET Interop API**（外部脚本/CI 集成）→ `tsmaster-dotnet-api.md`。**CAPL 语言参考**（CANoe 脚本编写）→ `capl-reference.md` + `templates/capl-resources/` + `capl-help/`。**原始 C API 头文件** → `templates/tsmaster-sdk-header.h`。
...
| 新 CAPL 函数/语法 | `references/capl-reference.md` + 追加 Excel |
| 新 CAPL 离线帮助 | 替换 `references/capl-help/` 中的 CHM |
| 新 CAPL 兼容层代码 | `templates/capl-resources/SysAndCan.h/.cpp` |
| 新 .NET API 类型/枚举 | `references/tsmaster-dotnet-api.md` |
| 新 SDK C API | `templates/tsmaster-sdk-header.h`（直接替换为最新版） |
