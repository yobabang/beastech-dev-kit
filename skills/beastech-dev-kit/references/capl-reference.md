...
```
references/capl-help/
├── CANeds.chm                    # CANoe DE Family 编译帮助（含完整 CAPL 函数参考，4.4MB）
├── StandaloneManager.chm         # Standalone Manager 帮助（1.3MB）
└── CAPLFunctions_HTML5.zip       # 5040 个 CAPL 函数详情页（20MB，解压后有完整 HTML 参考）

templates/capl-resources/
├── 函数(CAPL函数映射).xlsx         # CAPL 函数快速索引（1669 函数 + 签名 + 中英文描述）
├── SysAndCan.h / .cpp             # CAPL 兼容层样板
└── MiniProgram_Extern_Scaffold.cpp
```

> **CHM 文件**（Windows 双击打开）和 **HTML5 ZIP**（解压后浏览器打开 `CAPLFunctions.htm`）都是 Vector 官方 CAPL 函数参考，5040 个详情页覆盖 50+ 功能域，每个函数有参数说明/返回值/示例代码。
